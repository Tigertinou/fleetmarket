...
<div {{ $attributes->merge(['class' => 'checkbox']) }} style="font-size:{{ $size === 'lg' ? '1em' : ($size === 'sm' ? '0.8em' : '0.9em') }};">
    <div class="flex flex-1 items-center gap-2">
        <input type="checkbox" id="id-{{ $name }}-{{ $value }}" name="{{ $name }}" value="{{ $value }}" data-label="{{ html_entity_decode(strip_tags($label)) }}" @if($checked) checked @endif @if($disabled) disabled @endif style="font-size:{{ $size === 'sm' ? '0.8em' : '1em' }}">
        @if ($label)
