...
<div {{ $attributes->merge(['class' => 'h-11 border-gray-300 border-1 cursor-pointer relative select-none']) }} x-data="{
         ...
         selectedLabels: [],
         change() {
            this.selectedLabels = Array.from($el.querySelectorAll('input[data-label]:checked')).map(input => input.dataset.label);
    ...
    <div class="flex items-center flex-1 h-full mx-4" x-on:click="open = ! open">
        <span class="flex-1 truncate" :class="selectedLabels.length ? '' : 'text-gray-400 italic'" x-text="selectedLabels.length ? selectedLabels.join(', ') : '{{ $placeholder }}'"></span>
        <span><span class="icon icon-chevron-down" :class="open ? 'icon-chevron-up' : 'icon-chevron-down'"></span></span>
    </div>
    <div x-show="open" x-cloak class="absolute z-10 w-full overflow-y-auto bg-white shadow-lg select-list outline outline-gray-200 top-full max-h-50 min-h-30" @click.outside="open = false">
