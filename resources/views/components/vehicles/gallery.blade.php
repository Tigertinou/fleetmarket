...
@if(isset($images) && count($images) > 0)
    <div x-data="{
            ...
            expand (event) {
                ...
                setTimeout(() => {
                    $el.children[0].scrollTo({
                        left: index * $el.children[0].clientWidth
                    });
                }, 100);
            },
            collapse (event) {
            ...
            next () {
                const maxScrollLeft = $el.children[0].scrollWidth - $el.children[0].clientWidth - 50;
                if (Math.ceil($el.children[0].scrollLeft) >= maxScrollLeft) {
                    $el.children[0].scrollTo({ left: 0, behavior: 'smooth' });
                } else {
                    $el.children[0].scrollBy({
                        left: $el.children[0].clientWidth-50,
                        behavior: 'smooth'
                    });
                }
            }
